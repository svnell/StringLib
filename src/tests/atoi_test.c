...
START_TEST(atoii_test_p) {
  char src[] = "12345";
  int test = 12345;
  ck_assert_int_eq(test, s21_atoii(src));
}
END_TEST
...
START_TEST(atol_test_p) {
  ...
  ck_assert_int_eq(test, s21_atol(src));
}
END_TEST
...
START_TEST(atoll_test_p) {
  ...
  ck_assert_int_eq(test, s21_atoll(src));
}
END_TEST
...
START_TEST(atos_test_p) {
  char src[] = "12345";
  short test = 12345;
  ck_assert_int_eq(test, s21_atos(src));
}
END_TEST
...
START_TEST(atod_test_p) {
  ...
  ck_assert_double_eq(test, s21_atod(src));
}
END_TEST
...
START_TEST(atohex_test_p) {
  char src[] = "123Ab";
  int test = 74667;
  ck_assert_int_eq(test, s21_atohex(src));
}
END_TEST
...
START_TEST(ato8_test_p) {
  char src[] = "12345";
  int test = 5349;
  ck_assert_int_eq(test, s21_ato8(src));
}
END_TEST
